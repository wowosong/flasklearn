...
class Role(db.Model):
    ...
    def insert_roles():
        roles={
            'User':(Permission.FOLLOW | Permission.COMMENT|Permission.WTRITE_ARTICLES,True),
            'Moderator':(Permission.FOLLOW|Permission.COMMENT|Permission.WTRITE_ARTICLES|Permission.MODERATE_COMMENTS,False),
...
class User(UserMixin,db.Model):
    ...
    def can(self,permissions):
        # print self.role.permissions,self.role
        # print self.role.permissions & permissions
        # print permissions,'sa'
    ...
    def unfollow(self,users):
        f=self.followed.filter_by(followed_id=users.id).first()
        if f:
            db.session.delete(f)
    ...
    def generate_fake(count=100):
        ...
        for i in range(count):
            u=User(email=forgery_py.internet.email_address(),username=forgery_py.internet.user_name(True),password=forgery_py.lorem_ipsum.word(),
                   confirmed=True,name=forgery_py.name.full_name(),location=forgery_py.address.city(),about_me=forgery_py.lorem_ipsum.sentence(),
